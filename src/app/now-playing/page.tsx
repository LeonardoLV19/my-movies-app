...
export default function NowPlayingMoviesPage() {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const [nowPlayingRes, favoritesRes] = await Promise.all([
          getNowPlayingMovies(),
          getFavoritesMovies(sessionId)
        ]);

        setData({
          movies: nowPlayingRes.results.slice(0, 50),
          loading: false,
          error: null
        });
  ...
  const handleFavorite = async (movieId: number) => {
    ...
    try {
      ...
      setFavorites(prev => new Set(prev)); // Revert
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-4 sm:p-8">
      <div className="max-w-7xl mx-auto">
        <Header title="En Cartelera" count={data.movies.length} />
        <MovieGrid
          movies={data.movies}
          favorites={favorites}
...
const Header = ({ title, count }: { title: string; count: number }) => (
  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 gap-4">
    <div>
      ...
      <p className="text-gray-400 mt-2">Películas actualmente en cines</p>
    </div>
